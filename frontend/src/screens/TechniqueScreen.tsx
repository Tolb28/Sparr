...
import { getTechniquesGrouped, getDrillsGrouped, getCombinationsGrouped } from '../api/references';
...
export default function TechniqueScreen() {
  ...
  return (
    <Box className="flex-1 bg-gray-50">
      {/* Header */}
      <HStack className="bg-white border-b border-gray-200 pt-12 px-3 items-center gap-2">
        ...
        <HStack className="flex-1 items-center gap-1">
          {(['techniques', 'drills', 'combinations'] as const).map((tab) => (
            ...
            >
              <Text className={`font-bold text-sm ${activeTab === tab ? 'text-blue-600' : 'text-gray-600'}`}>
      ...
      <VStack className="flex-1">
        {loading && !refreshing ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <CategorySection categoryName={item.categoryName} items={item.items} onItemPress={handleItemPress} />
            )}
      ...
      >
        <VStack className="flex-1 pt-20 bg-white">
          <VStack className="px-4 pb-4">
            <Pressable
              ...
              className={`py-3 px-3 rounded-lg mb-2 ${selectedCategory === null ? 'bg-blue-100' : ''}`}
            >
              <Text className={`text-base font-semibold ${selectedCategory === null ? 'text-blue-600' : 'text-gray-700'}`}>
                All Categories
              </Text>
            </Pressable>
            
            <Text className="text-sm text-gray-500 font-semibold px-3 py-2 uppercase">Categories</Text>
            ...
            {categories.map((category: string) => (
              <Pressable
                ...
                className={`py-3 px-4 rounded-lg mb-1 ${selectedCategory === category ? 'bg-blue-100' : ''}`}
              >
                <Text className={`text-base font-medium ${selectedCategory === category ? 'text-blue-600' : 'text-gray-700'}`}>
                  {category}
                </Text>
              </Pressable>
            ))}
