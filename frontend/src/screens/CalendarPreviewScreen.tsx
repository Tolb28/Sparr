  ...
  return (
    <View style={[styles.root, { backgroundColor: c.background.secondary }]}>
      ...
      <View style={[styles.header, { paddingTop: (insets.top || 0) + 8, borderBottomColor: c.border.light }]}>
        ...
        <View style={styles.headerCenter}>
          ...
          {calendar?.creator_name && (
            <Text style={[styles.headerSub, { color: c.text.tertiary }]}>by {calendar.creator_name}</Text>
          )}
        </View>
        <View style={{ width: 38 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <GlassCard variant="medium" radius={14} padding={16}>
          <View style={styles.infoRow}>
            <View style={styles.infoBadge}>
              <Ionicons name="calendar-outline" size={16} color={c.primary.main} />
              <Text style={[styles.infoBadgeText, { color: c.text.secondary }]}>{items.length} day{items.length !== 1 ? 's' : ''}</Text>
            </View>
            <View style={styles.infoBadge}>
              <Ionicons
                name={calendar?.privacy === 'public' ? 'globe-outline' : 'lock-closed-outline'}
                size={14} color={c.text.tertiary}
              />
              <Text style={[styles.infoBadgeText, { color: c.text.secondary }]}>
            ...
            {calendar?.calendar_type === 'day' && calendar?.num_weeks > 1 && (
              <View style={styles.infoBadge}>
                <Text style={[styles.infoBadgeText, { color: c.text.secondary }]}>{calendar.num_weeks}wk rotation</Text>
              </View>
            )}
          </View>
        </GlassCard>
        ...
        {/* Day-by-day schedule */}
        <Text style={[styles.sectionTitle, { color: c.text.primary }]}>Schedule</Text>
        {items.length === 0 ? (
          <GlassCard variant="medium" radius={12} padding={20}>
            <Text style={[styles.emptyText, { color: c.text.tertiary }]}>No trainings in this calendar yet.</Text>
          </GlassCard>
              ...
              return (
                <View key={key} style={styles.dayRow}>
                  ...
                  <View style={styles.dayContent}>
                    {numWeeks > 1 && (
                      <Text style={{ color: c.text.tertiary, fontSize: 10, fontWeight: '600', marginBottom: 2 }}>
                        Week {wn}
                      </Text>
                    )}
                      ...
                      <View key={item.id_training_calendar_trainings || idx}>
                        {item.training ? (
                          ...
                          <GlassCard variant="default" radius={10} padding={12}>
                            <Text style={[styles.restText, { color: c.text.tertiary }]}>Rest Day</Text>
                          </GlassCard>
                        )}
                        {item.start_time && (
                          <Text style={{ color: c.text.tertiary, fontSize: 10, marginTop: 2 }}>
      ...
      <View style={[styles.bottomBar, { paddingBottom: (insets.bottom || 0) + 12, backgroundColor: c.background.secondary, borderTopColor: c.border.light }]}>
        <SparrButton
